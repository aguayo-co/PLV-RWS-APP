...
.upfile__item-wrap
  .upfile__item
    a.upfile__delete.i-x(
      ...
      span.hide Eliminar
    span.help(v-if="errorLog") {{ errorLog }}
    template(v-if="showCroppa")
      .upfile__label
        .upfile__text.i-upload(v-if="mqTablet") Arrastra una foto o
    ...
    img.upfile__img(:src="initialImage", v-else-if="initialImage")
  ...
  .upfile__controls
    ...
    template(v-else-if="initialImage")
      button.btn-tag(type="button", @click.prevent="editImage = true") Modificar esta imagen
</template>
